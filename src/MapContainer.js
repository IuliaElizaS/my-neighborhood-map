      ...
      <section className = "map-container" tabIndex = "0">
          ...
          {/* displays the markers on the page according to the user option */}
            ...
            .map(mapMarker => {
              ...
              {(this.props.selectedMarker !== undefined) ? (
                  <InfoWindow
                      visible = {this.props.showingInfoWindow}
                      marker = {this.props.selectedMarker}
                      markerPhoto = {this.props.markerPhoto}
                      >
                            { (this.props.marker !== undefined) ? (
                               <dialog className = "info" aria-label = "more informations about the place" >
                                <h3 className = "placeName">{this.props.marker.name}</h3>
                                <p aria-label = 'address of the selected place' className = "address" > {this.props.marker.location.address} Baia Mare, Romania</p>
                                ...
                                {(this.props.markerPhoto.items !== undefined && this.props.markerPhoto.items.length > 0) ? (
                                    <img className = "picture" src = {`${this.props.markerPhoto.items[0].prefix}height36${this.props.markerPhoto.items[0].suffix}`} alt = {this.props.marker.name}/>
                                ) : (
                                    <img className = "picture" src = "http://via.placeholder.com/50x36/ffe99b/282c4b?text=No+Image" alt = "a blank placeholder"/>
                                )}
                               </dialog>
                             ) : (
                               console.log ("Ups, there are no data to display in InfoWindow")
                             )
                            }
                        </InfoWindow>
                  ) : (
