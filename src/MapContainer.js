  ...
  render() {
    return(
      <section className="map-container">
          <Map className="map" role="application" aria-label="Map of Baia Mare, Romania, with places marked on it" google={this.props.google} onClick={this.props.closeInfoWindow} markerIcon = {this.props.markerIcon} markerDescription = {this.props.markerDescription} activateMarker={this.props.activateMarker} closeInfoWindow={this.props.closeInfoWindow} infoVisibility = {this.props.infoVisibility} selectedMarker = {this.props.selectedMarker} allMarkers = {this.props.allMarkers} zoom={14.25} initialCenter={{
              lat: 47.6573916, lng: 23.5652344}} styles = {[{featureType: 'poi', stylers: [{'visibility': 'off'}]}]}>
            { /*maps over the mapMarkers array and displays each marker on the page*/}
            {this.props.allMarkers.map(mapMarker => {
                return(
                  <Marker key= {mapMarker.id} role="button" aria-label="marker of the place" className="placeMarker" markerIcon = {this.props.markerIcon} onClick={(evt) => this.props.activateMarker(mapMarker, evt)}
                      name={mapMarker.name} position={{lat: mapMarker.location.lat, lng: mapMarker.location.lng}} icon={{
                  ...
                  {<div role="contentinfo" aria-label="more informations about the place" className="info">
                      <InfoWindow key= {mapMarker.location.lat} onClose={this.props.closeInfoWindow} markerDescription = {this.props.markerDescription} visible={this.props.infoVisibility} >
                            ...
                            (mapMarker.location.address) ? (
                                <span aria-label='address of the selected place' className="address" >{mapMarker.location.address}, Baia Mare, Romania</span>
                            ) : (
                            ...
                            (this.props.markerDescription.length > 1) ? (
                                <p role='textbox' aria-label='short description of the place' className="description">{this.props.markerDescription}</p>
                            ) : (
